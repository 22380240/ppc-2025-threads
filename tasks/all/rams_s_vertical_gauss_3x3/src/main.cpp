...
bool rams_s_vertical_gauss_3x3_all::TaskAll::PreProcessingImpl() {
  if (world_.rank() == 0) {
    ...
    kernel_ = std::vector<float>(reinterpret_cast<float*>(task_data->inputs[1]), reinterpret_cast<float*>(task_data->inputs[1]) + task_data->inputs_count[2]);
...
bool rams_s_vertical_gauss_3x3_all::TaskAll::ValidationImpl() {
  return world_.rank() != 0 || (task_data->inputs_count[2] == 9 && (task_data->inputs_count[0] * task_data->inputs_count[1] * 3) == task_data->outputs_count[0]);
}
...
bool rams_s_vertical_gauss_3x3_all::TaskAll::RunImpl() {
  ...
  for (size_t i = 0; i < world_size; i++) {
    sendcounts[i] = static_cast<int>(avg_to_send + (i < extra_to_send ? 1 : 0) + (i == 0 || i == world_size - 1 ? world_size == 1 ? 0 : 1 : 2)) * 3;
    recvcounts[i] = static_cast<int>(avg_to_send + (i < extra_to_send ? 1 : 0) + (i == 0 || i == world_size - 1 ? world_size == 1 ? -2 : -1 : 0)) * 3;
    if (i > 0) {
  ...
  for (size_t y = 0; y < height_; y++) {
    boost::mpi::scatterv(world_, input_.data() + (y * width_ * 3), sendcounts, displs, local_input.data() + (y * local_width * 3), local_width * 3, 0);
  }
  ...
  for (size_t x = std::max(left, size_t(1)); x < right; x++) {
    for (size_t y = 1; y < height_ - 1; y++) {
      for (size_t i = 0; i < 3; i++) {
        local_output[(y * local_width + x) * 3 + i] = std::clamp(
            static_cast<int>(std::round(
#define INNER(Y_SHIFT, X_SHIFT) local_input[((y + (Y_SHIFT)) * local_width + (x + (X_SHIFT))) * 3 + i] * kernel_[4 + 3 * (Y_SHIFT) + (X_SHIFT)]
...
#undef INNER
  ...
  for (size_t y = 1; y < height_ - 1; y++) {
    boost::mpi::gatherv(world_, local_output.data() + (y * local_width + 1) * 3, local_out_width, output_.data() + (y * width_ + 1) * 3, recvcounts, 0);
  }
