...
class RamsSVerticalGauss3x3StlTest : public ::testing::TestWithParam<std::tuple<uint32_t, uint32_t, std::vector<uint8_t>, std::vector<float>, std::vector<uint8_t>>> {
};
...
static void RunTest(uint32_t width, uint32_t height, std::vector<uint8_t>&in, std::vector<float>&kernel, std::vector<uint8_t>&expected) {
  ...
  rams_s_vertical_gauss_3x3_stl::TaskStl test_task(task_data);
...
TEST(rams_s_vertical_gauss_3x3_stl, test_with_fixture) {
  cv::Mat img = cv::imread(ppc::util::GetAbsolutePath("stl/rams_s_vertical_gauss_3x3/data/flower.png"));
  cv::Mat img_expected = cv::imread(ppc::util::GetAbsolutePath("stl/rams_s_vertical_gauss_3x3/data/flower-blurred.png"));
