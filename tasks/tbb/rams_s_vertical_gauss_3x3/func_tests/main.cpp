...
class RamsSVerticalGauss3x3TbbTest : public ::testing::TestWithParam<std::tuple<int, int, std::vector<uint8_t>, std::vector<float>, std::vector<uint8_t>>> {
};

static void RunTest(int width, int height, std::vector<uint8_t>&in, std::vector<float>&kernel, std::vector<uint8_t>&expected) {
  ...
  rams_s_vertical_gauss_3x3_tbb::TaskTbb test_task(task_data);
...
TEST_P(RamsSVerticalGauss3x3TbbTest , p) {
...
INSTANTIATE_TEST_SUITE_P(
  rams_s_vertical_gauss_3x3_tbb_test,
  RamsSVerticalGauss3x3TbbTest,
  ::testing::Values(
    std::tuple(
      0, 0,
      std::vector<uint8_t>{},
...
TEST(rams_s_vertical_gauss_3x3_tbb, test_with_fixture) {
  cv::Mat img = cv::imread(ppc::util::GetAbsolutePath("tbb/rams_s_vertical_gauss_3x3/data/flower.png"));
  cv::Mat img_expected = cv::imread(ppc::util::GetAbsolutePath("tbb/rams_s_vertical_gauss_3x3/data/flower-blurred.png"));
