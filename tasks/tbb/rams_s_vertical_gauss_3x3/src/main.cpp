...
bool rams_s_vertical_gauss_3x3_tbb::TaskTbb::ValidationImpl() {
  return task_data->inputs_count[2] == 9 && (task_data->inputs_count[0] * task_data->inputs_count[1] * 3) == task_data->outputs_count[0];
}
  ...
  oneapi::tbb::parallel_for(size_t(1), size_t(width_ - 1), [&](size_t x) {
    for (size_t y = 1; y < height_ - 1; y++) {
      for (size_t i = 0; i < 3; i++) {
        output_[(y * width_ + x) * 3 + i] = std::clamp(
            static_cast<int>(std::round(
#define INNER(Y_SHIFT, X_SHIFT) input_[((y + (Y_SHIFT)) * width_ + (x + (X_SHIFT))) * 3 + i] * kernel_[4 + 3 * (Y_SHIFT) + (X_SHIFT)]
