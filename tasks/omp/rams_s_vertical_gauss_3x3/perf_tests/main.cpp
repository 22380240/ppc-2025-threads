...
static void RunTest(bool pipeline) {
  ...
  auto test_task = std::make_shared<rams_s_vertical_gauss_3x3_omp::TaskOmp>(task_data);
...
TEST(rams_s_vertical_gauss_3x3_omp, test_pipeline_run) {
  RunTest(true);
}
TEST(rams_s_vertical_gauss_3x3_omp, test_task_run) {
  RunTest(false);
}
