...
class RamsSVerticalGauss3x3OmpTest : public ::testing::TestWithParam<std::tuple<int, int, std::vector<uint8_t>, std::vector<float>, std::vector<uint8_t>>> {
};
...
static void RunTest(int width, int height, std::vector<uint8_t>&in, std::vector<float>&kernel, std::vector<uint8_t>&expected) {
  ...
  rams_s_vertical_gauss_3x3_omp::TaskOmp test_task(task_data);
...
INSTANTIATE_TEST_SUITE_P(
  rams_s_vertical_gauss_3x3_omp_test,
  RamsSVerticalGauss3x3OmpTest,
  ::testing::Values(
    std::tuple(
      0, 0,
      std::vector<uint8_t>{},
...
TEST(rams_s_vertical_gauss_3x3_omp, test_with_fixture) {
  cv::Mat img = cv::imread(ppc::util::GetAbsolutePath("omp/rams_s_vertical_gauss_3x3/data/flower.png"));
  cv::Mat img_expected = cv::imread(ppc::util::GetAbsolutePath("omp/rams_s_vertical_gauss_3x3/data/flower-blurred.png"));
